 ...
 * @returns
 */
function checkAndInsertBoolean(str : string, type : string, _default : boolean | null = null) : boolean | null{
    if(/^true$/i.test(str)){
        return true;
 ...
 * @returns
 */
export async function addQiitaParameter(editor : vscode.TextEditor, qiitaPrm : QiitaParameter) : Promise<QiitaParameter | null>{

	// タイトルが未設定 or 空白
	if(!(qiitaPrm.title && qiitaPrm.title.length > 0)){
		qiitaPrm.title = "";
		await vscode.window.showInputBox({placeHolder :"記事のタイトル。1~255文字必要です。"}).then(e => {
			if(!e || e.length === 0){
				vscode.window.showErrorMessage("タイトルの文字が入力されませんでした。");
			}else if(e.length > 255){
				vscode.window.showErrorMessage("タイトルの文字が長すぎます。255文字以内に納めてください。：" + e);
			}else{
				qiitaPrm.title = e;
			}
			return;
	...
	if(qiitaPrm.tags.length === 0){
		...
		await vscode.window.showInputBox(options).then(e => {
			if(e){
				const tags = e.split(/[\s,]+/).map(e => e.trim()).filter(v => v !== "");
				if(tags.length > 5){
 ...
 * @param _default
 */
export function addQiitaParameterToTempleteDefault(qiitaPrm : QiitaParameter, _default : TypeQiitaTempleteDefault) : void{
	if(qiitaPrm.private === null){
		if(_default === "private"){
			qiitaPrm.private = true;
